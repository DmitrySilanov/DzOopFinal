...
public class ViewCalculator {
    ...
    public void run(){
        while (true){
            ComplexOperation firstArg = promptComplex("Введите пеpвый аpгумент: ");
            Calculable calculator = calculableFactory.create(firstArg);
            while (true){
                String cmd = prompt("Введите команду (+, *, :, -, =): ");
                if (cmd.equals("+")){
                    ComplexOperation arg = promptComplex("Введите втоpой аpгумент: ");
                    calculator.sum(arg);
                    continue;
                }
                if (cmd.equals("*")){
